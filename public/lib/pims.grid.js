...
(function (window, $) {
    ...
    // Declare app level module which depends on filters, and app.services
    var ngGridServices = angular.module('ngGrid.services', []);
    var ngGridDirectives = angular.module('ngGrid.directives', []);
    var ngGridFilters = angular.module('ngGrid.filters', []);
    // initialization of app.services into the main module
    ...
    var ngMoveSelectionHandler = function ($scope, elm, evt, grid) {
        ...
        if ($scope.showOptionMenu) {
            angular.forEach($scope.optionMenu, function (a) {
                if (a.tecla == charCode) {
                    $scope.optionSelect(a.accion);
                }
            });
        }
    ...
    angular.module('ngGrid.services').factory('$domUtilityService', ['$utilityService', function ($utils) {
        ...
        var getWidths = function () {
            ...
            $testContainer.append('<span style="font-family: Verdana, Helvetica, Sans-Serif; font-size: 10px;"><strong>M</strong></span>');
            //ver2.5 arriba font-size era 14
    ...
    var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
        ...
        //usamos el nuevo colDef.tip ... lo formateamos entre paréntesis siempre que esté presente... si no vacio   
        self.tip = (colDef.tip === undefined) ? "" : '(' + colDef.tip + ')';
        ...
        self.setVars = function (fromCol) {
            ...
            self.tip = fromCol.tip;                     //todo: : pending ver si llega siempre
    ...
    /// <reference path="../app.services/SortService.js" />
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
        var defaults = {
            ...
            */
            ...
            headerRowHeight: 25,    /* pasa a default de 25 (era de 30)   20140807 */
            ...
            //new   20140807 - habilita boton de opciones izquierdo
            ...
            //new 20141120, permite agrandar achicar la grilla, es parte del footer asi que tiene q estar showFooter activo
            sizingAllowed: false,
            sizingOrigHt: 0,
            ...
            i18n: 'es',  /*modif default by   20140807*/
    ...
    self = this;
        ...
        $scope.sizingAllowed = self.config.sizingAllowed;               // new   20141120
        $scope.sizingOrigHt = self.config.sizingOrigHt;                 // new   20141120 -- original h fo the viewport
        $scope.sizingStep = self.config.sizingStep;                     // new   20141120  -- px de avance del sizing
        $scope.sizingMaxHt = self.config.sizingMaxHt;                   // new   20141120  -- max-height al sizear
        $scope.footerRowHeight = $scope.showFooter ? self.config.footerRowHeight : 0;
        $scope.showColumnMenu = self.config.showColumnMenu;
        $scope.showOptionMenu = self.config.showOptionMenu;     // new   20140807 
        $scope.optionMenu = self.config.optionMenu;             // new   20140813
        ...
        //new   20141120 sizing
        $scope.sizeIt = function (level) {
            var e = self.$viewport.outerHeight();
            var f = $scope.sizingStep || 100;
            var g = $scope.sizingMaxHt || (f * 5);

            // determina por primera vez el h original
            if (!$scope.sizingOrigHt) { $scope.sizingOrigHt = self.$viewport.outerHeight(); }
            ...
            switch (level) {
                case 0:
                    self.$viewport.css('min-height', ($scope.sizingOrigHt) + 'px');
                    self.$viewport.css('height', ($scope.sizingOrigHt) + 'px');
                    break;
                case 1:
                    self.$viewport.css('min-height', (e + f) + 'px');
                    self.$viewport.css('height', (e + f) + 'px');
                    break;
                case 2:
                    self.$viewport.css('min-height', (e - f) + 'px');
                    self.$viewport.css('height', (e - f) + 'px');
        ...
        /* new   20140813 - reacciona a un menu manejable desde el controller de la vista 
                               y desde aqui dentro (desde la grilla) reacciona a teclas relacionadas al menu */
        $scope.optionSelect = function (opt) {
            switch (opt) {
                case 'multi':
                    $scope.multiSelect = !$scope.multiSelect;;
                    break;
                case 'select':
                    $scope.toggleSelectAll(true, false);
                    break;
                case 'deselect':
                    $scope.toggleSelectAll(false, false);
                    break;
                case 'filter':
                    $scope.toggleSelectAll(true, true);
                    break;
                default:
                    $scope.domAccessProvider.grid.config.optionCallBack(opt);
                    //config.afterSelectionChangeCallback();
            }
        };

        $scope.toggleSelectAll = function (state, selectOnlyVisible) {
            //new:  solo si esta habilitado el multiselect
    ...
    var ngSelectionProvider = function (grid, $scope, $parse) {
        var self = this;
        self.multi = $scope.multiSelect; //grid.config.multiSelect;        modif by   20140812 multiSelect se vuelve on-demand
        ...
        self.ChangeSelection = function (rowItem, evt) {
            ...
            self.multi = $scope.multiSelect;                                //new   20140812
    ...
    window.ngGrid.i18n['en'] = {
        ...
        ngTtipOptionMenu: 'Options',
        ngTtipColumMenu: 'Columns and grouping'   /*new  */
    };
    window.ngGrid.i18n['es'] = {
        ngAggregateLabel: 'items',
        ngGroupPanelDescription: 'Arrastre un encabezado aquí y suéltelo para agrupar por esa columna.',
        ngSearchPlaceHolder: 'Buscar...',
        ngMenuText: 'Elegir columnas:',
        ngShowingItemsLabel: 'Items Visibles:',
        ngTotalItemsLabel: 'Total Items:',
        ngSelectedItemsLabel: 'Items Seleccionados:',
        ...
        ngTtipOptionMenu: 'Opciones',                                   /*new estos  ultimos todo: : ver resto idiomas*/
        ngTtipColumMenu: 'Columnas y agrupamiento',
        ngTtipSize0: 'Restaurar',
        ngTtipSize1: 'Agrandar',
        ngTtipSize2: 'Achicar'
    ...
    angular.module("ngGrid").run(["$templateCache", function ($templateCache) {
        ...
        $templateCache.put("footerTemplate.html",
          ...
          "            <span> &nbsp; &nbsp; </span>" +
          "            <span ng-show=\"multiSelect\" class=\"ngLabel\">{{i18n.ngSelectedItemsLabel}} {{selectedItems.length}}</span>" +
          "        </div>" +
          //       modif   20140813 seleccionados al lado de la cantidad total y los filtrados. aqui abajo bkp de lo anterior
          ...
          "        </div>" +
          "    </div>" +
          //   parte de tamaño de grilla (new 20141120)
          "    <div class=\"ngSizeableContainer\"  ng-show=\"sizingAllowed\" >" +
          "        <div class=\"ngSize0Button\" tooltip-placement=\"up\" tooltip=\"{{i18n.ngTtipSize0}}\" ng-click=\"sizeIt(0)\">" +
          "            <div class=\"ngSize0ButtonArrow\"></div>" +
          "        </div>" +
          "        <div class=\"ngSize1Button\" tooltip-placement=\"up\" tooltip=\"{{i18n.ngTtipSize1}}\" ng-click=\"sizeIt(1)\">" +
          "            <div class=\"ngSize1ButtonArrow\"></div>" +
          "        </div>" +
          "        <div class=\"ngSize2Button\" tooltip-placement=\"up\" tooltip=\"{{i18n.ngTtipSize2}}\" ng-click=\"sizeIt(2)\">" +
          "            <div class=\"ngSize2ButtonArrow\"></div>" +
          "        </div>" +
          "    </div>" +
          "</div>"
        );
        ...
        /* modif   20140807 - 1. agregué tooltip al boton de menu 
                                 2. columnas muestran campo tip entre paréntesis  (col.tip)
                                 3. separacion entre grouping y nombre 
                                 4. nuevo menu para opciones: seleccionar/deseleccionar y otras particulares todo: : pending variable idioma 
        */

        $templateCache.put("menuTemplate.html",
          "<div ng-show=\"showOptionMenu\" class=\"ngOptionButton dropdown-toggle\" tooltip-placement=\"up\" tooltip=\"{{i18n.ngTtipOptionMenu}}\" >" +
          "    <div class=\"ngHeaderButtonArrow\"></div>" +
          "</div>" +
          "<ul class=\"dropdown-menu\" role=\"menu\"   >" +
          "    <li class=\"{{opt.accion}}divider\" ng-repeat=\"opt in optionMenu\" ng-click=\"optionSelect(opt.accion)\" ><a>{{opt.opcion}}</a></li>" +
          "</ul>" +
          "<div ng-show=\"showColumnMenu || showFilter\"  class=\"ngHeaderButton\" tooltip-placement=\"up\" tooltip=\"{{i18n.ngTtipColumMenu}}\" ng-click=\"toggleShowMenu()\">" +
          "    <div class=\"ngHeaderButtonArrow\"></div>" +
          "</div>" +
          ...
          "                <label><input ng-disabled=\"col.pinned\" type=\"checkbox\" class=\"ngColListCheckbox\" ng-model=\"col.visible\"/>{{col.displayName + ' ' + col.tip }} &nbsp; &nbsp; &nbsp; &nbsp; </label>" +
          "				<a title=\"Agrupar por\" ng-class=\"col.groupedByClass()\" ng-show=\"col.groupable && col.visible\" ng-click=\"groupBy(col)\"></a>" +
